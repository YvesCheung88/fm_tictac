...
export function check_board(board: Board) {
	// . = n'importe quel caractère et un seul

	// le regex est une representation visuel des différentes solutions
	if (board.join('').match('(^XXX|^...XXX|^......XXX|^X..X..X..|^.X..X..X.|^..X..X..X|^..X.X.X..|^X...X...X)'))
		return 'X'
	else if (board.join('').match('(^OOO|^...OOO|^......OOO|^O..O..O..|^.O..O..O.|^..O..O..O|^..O.O.O..|^O...O...O)'))
		return 'O'
	return null
}

export function check_is_full(board: Board) {
	/**
